namespace Serenity {
    ...
    @Decorators.registerClass('Serenity.MultiEditFieldSelect')
    class MultiEditFieldSelect extends Select2Editor<any, PropertyItem> {
        ...
        emptyItemText() {
            if (Q.isEmptyOrNull(this.value)) {
                return Q.text('Controls.MultiEditPanel.SelectField');
            }
    ...
    @Decorators.registerClass('Serenity.MultiEditOperatorSelect')
    class MultiEditOperatorSelect extends Select2Editor<any, MultiEditOperator> {
        constructor(hidden: JQuery, source: MultiEditOperator[]) {
            super(hidden);

            for (var op of source) {
                var title = Q.coalesce(op.title, Q.coalesce(
                    Q.tryGetText("Controls.MultiEditPanel.OperatorNames." + op.key), op.key));
    ...
    export class MultiEditPanel extends MultiEditWidgetBase<any> {
        ...
        updateRowsFromStore() {
            ...
            for (var item of items) {
                ...
                divl.children('.andor').toggleClass('or', !!item.isOr)
                    .text(Q.text((!!item.isOr ? 'Controls.MultiEditPanel.Or' :
                        'Controls.MultiEditPanel.And')));

                var fieldSelect = row.children('div.f')
                    .find('input.field-select').getWidget(MultiEditFieldSelect);

                fieldSelect.value = item.field;
                this.rowFieldChange(row);
                var operatorSelect = row.children('div.o')
                    .find('input.op-select').getWidget(MultiEditOperatorSelect);

                operatorSelect.set_value(item.operator);
                this.rowOperatorChange(row);

                var filtering = this.getMultiEditingFor(row);
                if (filtering != null) {
        ...
        protected initButtons(): void {
            this.byId('AddButton').text(Q.text('Controls.MultiEditPanel.AddMultiEdit'))
                .click((e) => this.addButtonClick(e));

            this.byId('SearchButton').text(Q.text('Controls.MultiEditPanel.SearchButton'))
                .click((e) => this.searchButtonClick(e));

            this.byId('ResetButton').text(Q.text('Controls.MultiEditPanel.ResetButton'))
            ...
            for (var i = 0; i < this.rowsDiv.children().length; i++) {
                try {
                    ...
                    var op = row.children('div.o').find('input.op-select')
                        .getWidget(MultiEditOperatorSelect).value;

                    if (op == null || op.length === 0) 
                        throw new (ss as any).ArgumentOutOfRangeException('operator',
                            Q.text('Controls.MultiEditPanel.InvalidOperator'));

                    var line: MultiEditLine = {};
        ...
        protected addEmptyRow(popupField: boolean) {
            ...
            var andor = parenDiv.children('a.andor').attr('title', Q.text('Controls.MultiEditPanel.ChangeAndOr'));
            if (isLastRowOr) {
                andor.addClass('or').text(Q.text('Controls.MultiEditPanel.Or'));
            }
            else {
                andor.text(Q.text('Controls.MultiEditPanel.And'));
            }

            andor.click((e) => this.andOrClick(e));

            row.children('a.delete')
                .attr('title', Q.text('Controls.MultiEditPanel.RemoveField'))
        ...
        protected populateOperatorList(row: JQuery): void {
            ...
            var opSelect = new MultiEditOperatorSelect(hidden, operators);
        ...
        protected rowOperatorChange(row: JQuery): void {
            ...
            var operatorSelect = row.children('div.o').find('input.op-select')
                .getWidget(MultiEditOperatorSelect);
        ...
        protected andOrClick(e: JQueryEventObject): void {
            ...
            andor.text(Q.text('Controls.MultiEditPanel.' +
