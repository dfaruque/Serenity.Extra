...
namespace _Ext.Reporting
{
    public class DynamicDataReport : IDataOnlyReport
    {
        ...
        public IEnumerable<string> DetailColumnList { get; set; }
        public Type ColumnsType { get; private set; }
        public Type DetailColumnsType { get; private set; }
        ...
        public DynamicDataReport(IEnumerable data, IEnumerable<string> columnList, Type columnsType, Type detailsColumnsType)
        {
            if (data == null)
                throw new ArgumentNullException("data");
            ...
            this.ColumnList = columnList ?? new List<string>();
            this.ColumnsType = columnsType;
            this.DetailColumnsType = detailsColumnsType;
            var detailColumnsProperties = Type.GetType(detailsColumnsType.FullName).GetProperties();
            var detailColsList = new List<string>();

            foreach (var item in detailColumnsProperties)
            {
                detailColsList.Add(item.Name);
            }
            DetailColumnList = detailColsList.AsEnumerable();
        }
        ...
        public List<ReportColumn> GetDetailColumnList()
        {
            var list = new List<ReportColumn>();
