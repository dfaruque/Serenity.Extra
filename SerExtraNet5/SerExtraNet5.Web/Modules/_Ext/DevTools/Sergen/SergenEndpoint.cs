...
namespace _Ext.DevTools.Endpoints
{
    ...
    public class SergenController : ServiceEndpoint
    {
        ...
        public ListResponse<SergenTable> ListTables(SergenListTablesRequest request)
        {
            ...
            Q.CheckNull(request, nameof(request));
            Q.CheckNullOrEmpty(request.ConnectionKey, "connectionKey");
        ...
        public ServiceResponse Generate(SergenGenerateRequest request)
        {
            ...
            Q.CheckNull(request, nameof(request));
            Q.CheckNullOrEmpty(request.ConnectionKey, "connectionKey");
            Q.CheckNull(request.Table, "table");
            Q.CheckNull(request.GenerateOptions, "table");
            Q.CheckNullOrWhiteSpace(request.Table.Tablename, "tableName");
            Q.CheckNullOrWhiteSpace(request.Table.Identifier, "identifier");
            Q.CheckNullOrWhiteSpace(request.Table.Module, "module");
