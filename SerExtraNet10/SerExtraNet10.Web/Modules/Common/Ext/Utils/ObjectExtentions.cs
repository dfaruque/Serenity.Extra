...
public static partial class ObjectExtentions
{
    ...
    public static string TimeSpanToDayCount(this TimeSpan? timeSpan)
    {
        if (timeSpan == null) return "";

        var spanPart = timeSpan.ToString().Split('.');
        if (spanPart.Length == 3)
        {
            return Convert.ToInt32(spanPart[0]).ToString("0#", new System.Globalization.CultureInfo("en-US"));
        }
        else
        {
            return "01";
