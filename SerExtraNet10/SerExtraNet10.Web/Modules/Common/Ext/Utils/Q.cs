...
public static partial class Q
{
    ...
    public static void CopyNonNullPropertyValues<TT, ST>(TT target, ST source)
    {
        var sourceProperties = typeof(ST).GetProperties().Where(prop => prop.CanRead);
        var targetProperties = typeof(TT).GetProperties().Where(prop => prop.CanRead && prop.CanWrite);

        foreach (var sourceProp in sourceProperties)
        {
            var targetProp = targetProperties.FirstOrDefault(f => f.Name == sourceProp.Name);
            if (targetProp != null)
            {
