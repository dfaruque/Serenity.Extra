...
namespace _Ext
{
    ...
    public class ReportController : Controller
    {
        ...
        public async Task<ActionResult> Render(string key, string opt, string ext, int? print = 0)
        {
            return await Execute(key, opt, ext, download: false, printing: print != 0);
        }

        public async Task<ActionResult> Download(string key, string opt, string ext)
        {
            return await Execute(key, opt, ext, download: true, printing: true);
        }

        private async Task<ActionResult> Execute(string key, string opt, string ext, bool download, bool printing)
        {
